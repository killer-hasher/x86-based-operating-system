...
int32_t syscall_halt (uint8_t status) {
    ...
    //set tss to parent
    tss.esp0 = EIGHT_MB - ((current_pid) * EIGHT_KB) - sizeof(int32_t);
    tss.ss0 = KERNEL_DS;
...
int32_t syscall_execute (const uint8_t* command){
    ...
    uint8_t buf[9 * 4096];
    read_data(file_data.inode_num,0, buf, 9 * 4096);
    memcpy((uint8_t*)0x08048000, buf, 9 * 4096);
    ...
    tss.esp0 = EIGHT_MB - ((current_pid) * EIGHT_KB) - sizeof(int32_t); //bottom of process block
    sti();
